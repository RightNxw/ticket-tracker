...
export function CardWithFormRemove() {
  ...
  async function onSubmit(event: React.FormEvent) {
    ...
    try {
      const response = await fetch(`http://localhost:3000/api/event?venueId=${venueId}`, {
        method: 'DELETE',
        headers: {
  ...
  return (
    <Card className="w-[350px] h-[350px]">
      ...
      <CardHeader>
        ...
        <CardDescription>Remove event so it doesn't monitor listing count and prices on VividSeats.</CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={onSubmit}>
          ...
          <CardFooter className="flex justify-center">
            <Button type="submit" disabled={!isFormValid}>Remove</Button>
          </CardFooter>
        </form>
      </CardContent>
    </Card>
